  ...
  return (
    <div className="flex flex-col gap-7 max-w-[472px] px-[20px]">
      {VentComplaintsCardInfo.map((VentComplaintCard) => (
        <VentComplaintsCard title={VentComplaintCard.title} description={VentComplaintCard.description} opacity={(100 - VentComplaintsCardInfo.indexOf(VentComplaintCard) * 19) / 100} key={VentComplaintCard.id} />
      ))}
    </div>
  );
};
