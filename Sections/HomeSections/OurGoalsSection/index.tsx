...
const OurGoalsSection: React.FC = (props) => {
  ...
  return (
    <div>
      <h1 data-aos="fade-up" className=" text-[22px] px-[30px] md:text-[32px] font-semibold lg:font-medium poppinsFont max-w-[800px] text-center mx-auto mt-[72px] mb-[90px]">
      ...
      <div className="mt-[50px] flex flex-col lg:flex-row justify-center items-center px-[30px] lg:px-0 gap-[20px] mb-[114px]">
        {goalCardInfo.map((card, index) => (
          <GoalCard title={card.title} description={card.description} iconPath={card.iconPath} key={index} />
        ))}
      </div>
    </div>
  );
